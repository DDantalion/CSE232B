...
def _is_cuda() -> bool:
    ...
    explicit_cuda_target = os.getenv("VLLM_TARGET_DEVICE") == "cuda"
    allow_precompiled_cuda = explicit_cuda_target and envs.VLLM_USE_PRECOMPILED
    return (VLLM_TARGET_DEVICE == "cuda" and (has_cuda or allow_precompiled_cuda)
...
def get_requirements() -> list[str]:
    ...
    if _no_device():
        ...
        cuda_major = None if cuda_version is None else cuda_version.split(".")[0]
        modified_requirements = []
        for req in requirements:
            if ("vllm-flash-attn" in req and cuda_major is not None
                    and cuda_major != "12"):
